...
abstract class RestApiInvoker {
    ...
    private static final Authenticator authenticator = (route, response) -> response.request().newBuilder()
            .header("Proxy-Authorization", Credentials.basic("admin","123"))
            .build();
    private static final Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("172.105.228.27", 3128));
    private static final OkHttpClient client = new OkHttpClient.Builder().proxy(proxy).proxyAuthenticator(authenticator).build();
